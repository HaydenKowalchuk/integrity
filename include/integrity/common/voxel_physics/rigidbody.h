...
static inline void RBDY_MarkActive(rigidbody* body) {
  body->_sleepFrameCount = 10 | 0;
}
static inline float RBDY_atRestX(rigidbody* body) {
  return body->resting[0];
}
static inline float RBDY_atRestY(rigidbody* body) {
  return body->resting[1];
}
static inline float RBDY_atRestZ(rigidbody* body) {
  return body->resting[2];
}
